...
describe('default project', () => {
  it('starts from Goldoni with the La locandiera sample file and real sample media assets', () => {
    ...
    expect(script).toContain('questo è un file di esempio e non viene registrato sul dispositivo')
    expect(script).toContain('importa un copione dallo Store di StageDesk')
    expect(script).toContain('> **Sinossi**')
    expect(script).toContain('### Personaggi')
    expect(script).toContain('| Personaggio | Descrizione |')
    expect(script).toContain('| MIRANDOLINA | D/A | 1-XV | |')
    ...
    expect(script.indexOf('### Personaggi')).toBeLessThan(script.indexOf('# Atto 1'))
    expect(script).not.toContain('title="Personaggi in scena"')
